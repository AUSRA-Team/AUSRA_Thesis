...
This file is auto-loaded by Claude Code when a session starts in this directory.
It captures the thesis restructure plan, decisions made, and where we stopped so
work can resume seamlessly.

- CRITICAL: Due to a parsing bug with Opus 4.8, you are strictly limited to ONE tool call per turn.
- Do NOT attempt to read multiple files, run multiple commands, or edit multiple files in a single response.
- If a task requires multiple tool calls (e.g., editing three different files), you must do them sequentially across multiple turns. 
...
## Workflow conventions for this thesis
- Edit `.tex` files directly, one section/block per step, keep each reviewable.
- Ground every claim in the real package files/parameters (read them, don't assume).
- For teammate-owned or not-yet-available content: insert heading/subheading + a
  `% [TODO: ...]` placeholder only, leave body empty.
- `@` file-picker is rooted at AUSRA_Thesis, so it won't show sibling ROS packages —
  Claude reads those by absolute path directly.
